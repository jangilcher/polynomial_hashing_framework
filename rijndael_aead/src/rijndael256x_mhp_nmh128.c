                               ...
                               const uint8_t nonce[NONCE_SIZE])
{
    ...
    rijndael256block(&((__m128i *)&keys)[4], roundkeys, iv[0]);
    iv[1][0] = iv[0][0];
    iv[2][0] = iv[0][0];
    iv[3][0] = iv[0][0];
    ...
    input[0] = zero.v;
    input[1] = zero.v;
    rijndael256block(&((__m128i *)&keys)[0], roundkeys, input);
    input[1] = one.v;
    rijndael256block(&((__m128i *)&keys)[2], roundkeys, input);

    inner_keys[0] = keys.inner;
...
#pragma region AD
    ...
    while (ad_len >= (2 * HASHBLCK_SIZE)) {
        h_msg[0].val[0] = _mm_loadu_si128(ad_ptr.v + 0);
        h_msg[1].val[0] = _mm_loadu_si128(ad_ptr.v + 1);
        ad_ptr.v += 2;
        ...
        if (blck_num == SPRBLCKSIZE) {
            ...
            field_mul_no_carry(&dacc, &acc, &keys.outer);
            blck_num = 0;
        }
    }
    if (ad_len > HASHBLCK_SIZE) {
        ...
        h_msg[0].val[0] = _mm_loadu_si128(ad_ptr.v + 0);
        memcpy(h_msg[1].val, ad_ptr.v + 1, ad_len - HASHBLCK_SIZE);
        ...
        memcpy(h_msg[0].val, ad_ptr.c, ad_len);
        ad_len = 0;
        ...
        if (to_enc > 0) {
            ...
            if (ct_r_ptr.v + 1 <= ct_ptr.v) {

                h_msg[1].val[0] = _mm_loadu_si128(ct_r_ptr.v);
                ct_r_ptr.v += 1;
            } else {
                memcpy(h_msg[1].val, ct_r_ptr.v, ct_ptr.c - ct_r_ptr.c);
                ct_r_ptr.c = ct_ptr.c;
            }
            field_add(&odd, &h_msg[1], &(inner_keys[blck_num + 1]));
...
#pragma region ENCRYPTION
    ...
    while (to_enc >= 4 * BLCKSIZE) {
        ...
        for (int i = 0; i < 4; i++) {
            h_msg[0].val[0] = _mm_loadu_si128(ct_r_ptr.v + 0);
            h_msg[1].val[0] = _mm_loadu_si128(ct_r_ptr.v + 1);
            field_add(&evn, &h_msg[0], &(inner_keys[blck_num + 0]));
            field_add(&odd, &h_msg[1], &(inner_keys[blck_num + 1]));
            field_mul_no_carry(&tmp, &evn, &odd);
            field_add_dbl(&dacc, &dacc, &tmp);
...
#pragma region HASH FINALIZE
    ...
    if (ct_r_ptr.c + (1 * HASHBLCK_SIZE) <= ct_ptr.c) {
        ...
        h_msg[0].val[0] = _mm_loadu_si128(ct_r_ptr.v);
        h_msg[1].val[0] = _mm_setzero_si128();
        memcpy(h_msg[1].val, ct_r_ptr.v + 1,
               ct_ptr.c - (ct_r_ptr.c + HASHBLCK_SIZE));
        ct_r_ptr = ct_ptr;
        PRINT_FIELD_ELEM(h_msg[0]);
        PRINT_FIELD_ELEM(h_msg[1]);
        field_add(&evn, &h_msg[0], &(inner_keys[blck_num + 0]));
        field_add(&odd, &h_msg[1], &(inner_keys[blck_num + 1]));
        PRINT_FIELD_ELEM(evn);
        PRINT_FIELD_ELEM(odd);
        ...
        h_msg[0].val[0] = _mm_setzero_si128();
        memcpy(h_msg[0].val, ct_r_ptr.c, ct_ptr.c - ct_r_ptr.c);
        ct_r_ptr = ct_ptr;
        field_add(&evn, &h_msg[0], &(inner_keys[blck_num + 0]));
        field_add(&odd, &len_blck, &(inner_keys[blck_num + 1]));
        field_mul_no_carry(&tmp, &evn, &odd);
...
#pragma endregion HASH FINALIZE
#endif
    field_add_mix(&dacc, &dacc, &keys.le);
    carry_round(&acc, &dacc);
    ...
    // PRINT_FIELD_ELEM(acc);
    memcpy(c + PADDED_LEN(m_len), &acc.val[0], TAG_SIZE);
    return;
}
