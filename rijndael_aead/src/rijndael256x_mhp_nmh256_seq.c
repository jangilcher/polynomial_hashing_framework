...
#include "rijndael256/rijndael256x_mhp_nmh256_seq.h"
...
void rijndael256x6x_mhp_nmh256_seq(size_t m_len, const uint8_t m[static m_len],
                                   size_t ad_len,
                                   ...
                                   const uint8_t nonce[NONCE_SIZE])
{
    ...
    rijndael256block(&key_buff[6].v, roundkeys, iv[0]);
    ...
    input[1] = two.v;
    rijndael256block(&key_buff[4].v, roundkeys, input);
    ...
    field_elem_t acc = {0};
    dfield_elem_t tmp = {0};
    field_elem_t outer_key_powers[4];
    ...
    unpack_and_encode_key(&outer_key_powers[0], key_buff[0].i8 + 32);
    unpack_and_encode_key(&le_key, key_buff[0].i8 + 64);
    unpack_and_encode_key(&wc_key, key_buff[6].i8);

    PRINT_FIELD_ELEM(inner_keys[0]);
    PRINT_FIELD_ELEM(outer_key_powers[0]);
    ...
    for (int i = 0; i < 3; i++) {
        field_mul(&outer_key_powers[i + 1], &outer_key_powers[i],
                  &outer_key_powers[0]);
    }
...
#pragma region HASH
    ...
    while (ad_len >= 2 * HASHBLCK_SIZE) {
        ...
        // {
        //     field_elem_t tacc;
        //     carry_round(&tacc, &dacc);
        //     _carry_round(&tacc, &tacc);
        //     _carry_round(&tacc, &tacc);
        //     _carry_round(&tacc, &tacc);
        //     PRINT_FIELD_ELEM(tacc);
        // }
        ...
        if (blck_num == SPRBLCKSIZE) {
            ...
            field_mul_no_carry(&dacc, &acc, &outer_key_powers[0]);
            blck_num = 0;
        }
    }
    ...
    if (ct_r_ptr.c + (HASHBLCK_SIZE) < ct_ptr.c) {
        HERE;
        DEBUG_PRINTF("ct_r_ptr.c + (HASHBLCK_SIZE) < ct_ptr\n");
...
void rijndael256x4x_mhp_nmh256_seq(size_t m_len, const uint8_t m[static m_len],
                                   size_t ad_len,
                                   const uint8_t ad[static ad_len],
                                   uint8_t c[static CTXT_LEN(m_len)],
                                   const uint8_t key[static KEY_SIZE],
